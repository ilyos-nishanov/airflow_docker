...
def max_number_find():
    ...
    check_table_query = f"""select max(number) from RISKDB.dbo.overdue_procents;
    """
...
def insert_into_mssql(df, table_name):
    ...
    # Define mapping of Pandas dtypes to SQL Server types
    dtype_mapping = {
        'int64': 'BIGINT',
        'float64': 'FLOAT',
    ...
    # Clean the DataFrame for SQL Server compatibility
        ...
        for contract in contracts_list:
            ...
            if isinstance(overdue_procents, dict):
                ...
                for overdue_procents_item in overdue_procents:
                    overdue_procent = overdue_procents_item.get('overdue_procent')
                    if isinstance(overdue_procent, dict):
...
# The `print(df)` statement in the provided code is displaying the DataFrame `df` in a tabular format. This means that it will output the contents of the DataFrame `df` to the console or standard output, allowing you to visually inspect the data in a structured table-like format. This can be helpful for debugging, data analysis, or simply viewing the processed data in a more organized manner.
# print(df)
