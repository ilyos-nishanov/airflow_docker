...
def upsert_to_sql_batch(df, product):
    ...
    for start in range(0, len(df), batch_size):
        ...
        PrimaryKeyColumn = 'GLOB_ID'
        create_temp_table_query = f"""

            IF OBJECT_ID({sql_table}, 'U') IS NULL
            CREATE TABLE {sql_table} ( {', '.join([f'[{col}] NVARCHAR(100)' for col in df.columns])} )
        ...
        merge_query = f"""
            ...
            WHEN MATCHED THEN
                UPDATE SET 
                    {', '.join([f'Target.{col} = Source.{col}' for col in df.columns if col != {PrimaryKeyColumn}])}
